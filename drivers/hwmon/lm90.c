...
#undef SELECT_REGS
	...
	i2c_smbus_write_byte_data(client, high, data->temp11[index] >> 8);
	if (data->flags & LM90_HAVE_REM_LIMIT_EXT)
		i2c_smbus_write_byte_data(client, low, data->temp11[index] & 0xff);
	lm90_select_remote_channel(data, 0);
}
...
static ssize_t lm90_sysfs_show_pec(struct device *dev, struct device_attribute *dummy,
			char *buf)
{
...
static ssize_t lm90_sysfs_set_pec(struct device *dev, struct device_attribute *dummy,
		       const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	long val;
